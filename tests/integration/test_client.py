...
class TestBedcaClientMockedRequests:
    ...
    def client_with_mock_session(self, mock_session):
        """Fixture providing a client with mocked session."""
        client = BedcaClient()
    ...
    def test_get_all_foods_success(self, client_with_mock_session, xml_search_response):
        ...
        client_with_mock_session.session.post.assert_called_once()
        call_args = client_with_mock_session.session.post.call_args
    ...
    def test_network_error_handling(self, client_with_mock_session):
        ...
        client_with_mock_session.session.post.side_effect = requests.ConnectionError("Network error")
...
class TestBedcaClientQueryGeneration:
    ...
    def client_with_captured_queries(self, xml_search_response, xml_food_response):
        ...
        def capture_post(url, headers=None, data=None):
            captured_queries.append(data)
            mock_response = Mock()
            
            # Return appropriate response based on query type
            if 'level="1"' in (data or ""):
                # Search query
                mock_response.text = xml_search_response
            elif 'level="2"' in (data or ""):
                # Detail query  
                mock_response.text = xml_food_response
            else:
...
<foodresponse>
        ...
        client.session.post = capture_post
...
class TestBedcaClientEdgeCases:
    ...
    def test_search_with_empty_string(self, mock_bedca_client):
        ...
        mock_response.raise_for_status = Mock()
        mock_bedca_client.session.post.return_value = mock_response
