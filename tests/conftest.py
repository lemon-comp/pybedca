...
def mock_session(mock_response):
    ...
    def _create_mock_session(response_text: str = None):
        session = Mock()
        if response_text:
            session.post.return_value = mock_response(response_text)
        return session
...
@pytest.fixture
def mock_bedca_client(mock_session):
    """Fixture providing a BedcaClient with mocked session."""
    client = BedcaClient()
...
def mock_requests_session():
    """Fixture providing a comprehensive mock requests session."""
    def _create_session(responses=None):
        """Create a mock session with configurable responses."""
        session = Mock()
        
        if responses:
            # Configure responses based on URL or data patterns
